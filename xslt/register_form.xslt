...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <h1 class="color-primary text-center"> <xsl:value-of select="/sqroot/header/info/title/." /></h1>

    <form class="form-horizontal" id="formregister" method="post">
      <fieldset>
        ...
        <div class="row">
          <div class="col-md-4 col-md-offset-8">
            <button class="btn btn-raised btn-primary btn-block mt-4" onclick="savethemeOPERAHOUSE('register', '00000000-0000-0000-0000-000000000000', '', 'formregister')">Register</button>
          </div>
        </div>
  ...
  <xsl:template match="formCol">
    <xsl:if test="count(../formCol) &lt; 2">
      <div class="col-md-12">
        <xsl:apply-templates select="formRows"/>
      </div>
    </xsl:if>
    <xsl:if test="count(../formCol) &gt; 1">
      <div class="col-md-6">
        <xsl:apply-templates select="formRows"/>
      </div>
    </xsl:if>
  ...
  <xsl:template match="textBox">
    <label id="{../@fieldName}caption" for="inputUser" class="col-md-3 control-label" style="margin:0px; margin-top:7px;">
  ...
  <xsl:template match="textArea">
    ...
    <div class="col-md-9">
      <textarea class="form-control" name="{../@fieldName}" id ="{../@fieldName}" data-type="textArea" style="max-width:100%; min-width:100%; min-height:55px;"
        ...
        <xsl:if test="../@isEditable=0 or (../@isEditable=2 and (/sqroot/body/bodyContent/form/info/GUID/. != '00000000-0000-0000-0000-000000000000'))">
          <xsl:attribute name="disabled">disabled</xsl:attribute>
        </xsl:if> <xsl:if test="../@isEditable='1'">
          <xsl:attribute name="placeholder">Please Enter <xsl:value-of select="titlecaption" /></xsl:attribute>
        </xsl:if>
  ...
  <xsl:template match="checkBox">
    ...
    </input><label id="{../@fieldName}caption" style="margin-left:10px;"><xsl:value-of select="titlecaption"/></label>
